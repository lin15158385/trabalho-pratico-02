    ...
    using System.Linq;
    using static trabalho_pratico_individual_02.Game1;
    ...
    namespace trabalho_pratico_individual_02
    {
        ...
        public class Zombie : Enemy
        {
            ...
            private float _attackFrameSpeed = 0.5f;
                ...
                foreach (var unit in units)
                {
                    if (this.Bounds.Intersects(unit.Bounds))
                    {
                        if (_attackTimer >= _attackCooldown)
                        {
                            _isAttacking = true;
                            _attackTimer = 0f;
                            _attackFrameIndex = 0f; // Reinicia animação de ataque
        ...
        private void AnimateAttack(GameTime gameTime)
        {
            ...
            // Aplica dano no frame 2
            if (!_damageAppliedThisAttack && (int)_attackFrameIndex == 2)
            {
        ...
        public class Skeleton : Enemy
        {
            ...
            public override void Update(GameTime gameTime, Vector2 target, List<Unit> units)
            {
                ...
                foreach (var arrow in Arrows.ToList())
                {
                    ...
                    foreach (var unit in units)
                    {
                        if (arrow.Bounds.Intersects(unit.Bounds))
                        {
                            unit.Health -= 10;
                            if (unit.Health <= 0 && !unit.IsDead)
                                unit.Die();
        ...
        public class Arrow
        {
            ...
            public void Draw(SpriteBatch spriteBatch)
            {
                ...
                spriteBatch.Draw(_texture, _position, null, Color.White, rotation,
                    new Vector2(_texture.Width / 2f, _texture.Height / 2f), 5f, SpriteEffects.None, 0f);
            }
